...
* posix/threads/mutex
  ...
  <br>Почему такие уточнения? Чтобы задача имела больший образовательный смысл. С одним мьютексов у вас не параллелится обработка данных (которая в этой задаче упрощена до примитивного +=). Атомарность изменения требуется, чтобы потренироваться 
